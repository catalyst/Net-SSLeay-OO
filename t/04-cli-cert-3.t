#!/usr/bin/perl
#
#  t/04-cli-cert-3.t - Test client certificates - degenerate cert
#
...
my $cert_dir = "$Bin/certs-bad";

my $ctx = Net::SSLeay::OO::Context->new;
$ctx->set_default_passwd_cb( sub {""} );
...
unless ($child_pid) {
	...
	$ctx->use_certificate_chain_file("$cert_dir/host.crt");
	$ctx->use_PrivateKey_file( "$cert_dir/host.key", FILETYPE_PEM,);
	...
	ok( $name && (!defined $name->cn), "extracted (lack of) CN OK" )
...
waitpid($child_pid, 0);
ok($? == 0, "child process exited gracefully");
if ($?) {
	if ($? > 0) {
		diag( "rc=$?; ". (($? >> 8) ? "exited with error code ".($?>>8)
			: "killed by signal ".($?&0x7f).($?&0x80?" (core dumped)":"")));
	}
	else {
		diag("\$? < 0 ($?)");
	}
